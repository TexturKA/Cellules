...
def operations(service: CelluesApplication, threads: list):
    draw = Draw(service.picture.image)
    ...
    while threads:
        ...
        for cell in service.cells.copy():
            ...
            if divinity_expression:
                ...
                draw.point(passed_step.xy, darkening(cell.clan.color, service.darkening_degree).rgb)
                draw.point(cell.pos.xy, cell.clan.color.rgb)

                if cell.action.steps_counter > service.lifetime:
                    draw.point(cell.pos.xy, darkening(cell.clan.color, 2).rgb)
